...
const {RNMonkeyWebServer} = NativeModules;
...
class StaticServer {
  ...
  start() {
    ...
    return RNMonkeyWebServer.perth_port(
      this.port,
      this.perthKey,
      this.perthPath,
      this.localOnly,
  ...
  stop() {
    this.running = false;
    return RNMonkeyWebServer.perth_stop();
  }
  ...
  isRunning() {
    return RNMonkeyWebServer.perth_isRunning().then(running => {
